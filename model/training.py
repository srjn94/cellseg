...
def train_sess(sess, model_spec, num_steps, writer, params):
    ...
    metrics_string = " ; ".join("{}: {:010.8f}".format(k, v) for k, v in metrics_val.items())
...
def train_and_evaluate(train_model_spec, eval_model_spec, model_dir, params, restore_from=None):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(begin_at_epoch, begin_at_epoch + params.num_epochs):
            ...
            eval_macro_f1_score = metrics['macro_f1_score']
            if eval_macro_f1_score >= best_eval_macro_f1_score:
                # Store new best accuracy
                best_eval_macro_f1_score = eval_macro_f1_score
                # Save weights
                best_save_path = os.path.join(model_dir, 'best_weights', 'after-epoch')
                best_save_path = best_saver.save(sess, best_save_path, global_step=epoch + 1)
                logging.info("- Found new best macro_f1_score, saving in {}".format(best_save_path))
